...
def scrap():
    ...
    def bot_send(data):
        ...
        with db_connection().cursor() as cursor:
            sql_query = "SELECT id, code FROM user"
            cursor.execute(sql_query)
            users = [{'id': element[0], 'chat_id':element[1]} for element in cursor.fetchall()]

            for user in users:
                sql_query = f"SELECT id, name FROM filters WHERE id_user={user['id']}"
                cursor.execute(sql_query)
                user_filters = [{'id': element[0], 'name': element[1]} for element in cursor.fetchall()]

                for filter in user_filters:
                    sql_query = f"SELECT id_option FROM filter_elements WHERE id_filter={filter['id']}"
                    cursor.execute(sql_query)
                    filter_option_id = [element[0] for element in cursor.fetchall()]

                    job_weight = 0
                    # skill
                    if 2 in filter_option_id:
                        sql_query = f"SELECT option_value FROM filter_elements WHERE id_option={2} AND id_filter={filter['id']}"
                        cursor.execute(sql_query)
                        filter_value = [int(element) for element in cursor.fetchone()[0][1:-1].split[', ']]
                        count = 0
                        flag = False
                        for skill in filter_value:
                            if skill in [element['id'] for element in data['tags']]:
                                flag = True
                                count += 1

                        if not flag:
                            continue
                        percent = count/len(filter_value) * 100

                    # price
                    flag = False
                    if data['price']['isFixed']:
                        if 1 in filter_option_id:
                            sql_query = f"SELECT option_value FROM filter_elements WHERE id_option={1} AND id_filter={filter['id']}"
                            cursor.execute(sql_query)
                            filter_value = [float(element.split(': ')[1]) for element in cursor.fetchone()[0][1:-1].split(', ')]





                    if 3 in filter_option_id:
                        sql_query = f"SELECT option_value FROM filter_elements WHERE id_option={3} AND id_filter={filter['id']}"
                        cursor.execute(sql_query)
                        if data['price']['isFixed']:
                            if min(filter_value) <= data['price']['cost'] <= max(filter_value):

                                job_weight += 1
                            elif min(filter_value) <= (data['price']['cost']['max']-data['price']['cost']['min'])/2 + data['price']['cost']['min'] <= max(filter_value):
                                job_weight += 1
                            else:
                                continue

                    # fixed price
                    elif 1 in filter_option_id
                    flag == True
                    if data['price']['isFixed']:
                        if 1 in filter_option_id:
            ...
            for filter in filters:
                ...
                for filter_element in filter_elements:
                    ...
                    if func == 'fixed_price':
                        if not data['price']['isFixed']:
                            flag = False
                        elif not check_filter_fixed_price(data['price']['cost'], filter_element['option_value']):
                            flag = False
                        else:
                            job_weight += 1
                        ...
                        if data['price']['isFixed']:
                            flag = False
                        elif not check_filter_hourly_price(data['price']['cost'], filter_element['option_value']):
                            flag = False
                        else:
                            job_weight +=1
                ...
                if not check_filter_percent_skill(percent_skill, filter_percent_skill) or not flag:
                    continue
